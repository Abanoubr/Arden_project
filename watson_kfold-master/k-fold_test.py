...
# This script will allow the user to perform a K-fold cross validation test on your Watson Assistant workspace 
#   
# 1. Connect to WA : credentials to connect to the right WA workspace<br>
# 2. Scan the workspace : download the GTs and intent labels of the entire workspace <br>
# 3. Test and train split : divide the dataset in test and train sets<br>
# 4. Create k workspaces : create k workspace and train it with the train-sets<br>
# 5. Analyse k-fold results : send the test set to the right workspace and analyse the results. <br>
# 6. Analyse the Report : focus on the performance of each intent.<br>
# 7. Incorrect Matches : analysis of different types of incorrect matches and possible solutions<br>
# 8. Highlight Possible `Confused` Intents : highlight possible overlapping intents<br>
# 
...
#  
# In this particular script we are going to use the stratified k-fold cross validation. This is a variation of KFold that returns stratified folds. The folds are made by preserving the percentage of samples for each class. It is used for unbalanced classes (e.g. intent dimensions that are not equals). 
# 
...
# ## Libraries 
#!pip install ibm-watson pandas numpy  matplotlib ipython seaborn sklearn
...
# 
# 1. Credentials to connect to WA : Log in to IBM cloud, Go to your resource list, select your WA service, and Click Show Credentials - based on what credentials you have, choose option1 (IAM) or option2 (Basic) to connect to your WA instance.<br>
# 2. Workspace : the ID of the workspace that you want to analyse. - Go to your WA instance, "Skills" tab, click on the three dots of the workspace/skill that you are interested, choose "View API details" and you will find the Workspace ID<br>
# 3. K-fold number: usually the common used folds number is five. but in can be higher (e.g. 10) or less (i.e. 3) depending on the workspace size.<br>
# 4. Threshold: this is the minimum level of confidence level that you are expecting in order to check if the input text is triggering the right intent. For example, if the threshold = 0.40 and the input phrase is triggering the correct intent with only 35% of confidence level, we would want to count it as a False Positive (not meeting the required conditions) and not as a True Positive, despite the intent is correct. The default in WA is 0.2. Less than 20% the utterance is classified as `irrelevant`. However, if you need to have a higher threshold it can be set here. 
